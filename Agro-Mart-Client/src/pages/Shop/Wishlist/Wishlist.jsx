...
const Wishlist = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      axiosSecure.get(`/wishlist/${user?.email}`).then((res) => {
            ...
            {wishlist?.map((item) => (
              ...
              >
                ...
                <td className="p-4 text-right">
                  <button onClick={() => handleAddToCart(item)} className="bg-green-700 hover:bg-yellow-300 hover:text-black text-white px-6 py-2 rounded-full font-semibold transition">
                    Add To Cart
                  </button>
                </td>
